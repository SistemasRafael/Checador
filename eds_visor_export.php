...
$sql = "SELECT 
        ...
        $mina_unidad = sqlsrv_fetch_array($eds_datos);
        $mina_nom = $mina_unidad['serie'];
/*if ($mina == "LC" or $mina == 'LC')
    $mina_nom = "LA COLORADA";
    else{
        if ($mina == "EC" or $mina == 'ec')
...
$sql = "EXEC [BiostarData_TA].[dbo].[_proc_marcajes_visorV2] '".$fecha_ini."','".$fecha_fin."',".$mina.", '".$empleado."'";
$eds_datos = sqlsrv_query( $eds_link, $sql);
...
$objSheet->setTitle('Checador '.$mina_nom);
 ...
 $objSheet->getCell('C2')->setValue('Departamento');
 $objSheet->getCell('D2')->setValue('Tipo');
 $objSheet->getCell('E2')->setValue('Checador');
 $objSheet->getCell('F2')->setValue('Fecha');
 $objSheet->getCell('G2')->setValue('Hora');
 ...
 $i = 3;
  while( $row = sqlsrv_fetch_array( $eds_datos) ) { 
    ...
    $objSheet->getCell('C'.$i)->setValue($row['departamento']);
    $objSheet->getCell('D'.$i)->setValue($row['tipo_empleado']);
    $objSheet->getCell('E'.$i)->setValue($row['terminal']);
    $objSheet->getCell('F'.$i)->setValue($row['fecha']);
