...
$sql = "EXEC [BiostarData_TA].[dbo]._proc_marcajes_roles '".$fecha_ini."','".$fecha_fin."',".$mina;
$eds_datos = sqlsrv_query( $eds_link, $sql);

$row_count = sqlsrv_num_rows( $eds_datos );

$objSheet = $objPHPExcel->getActiveSheet();
$objSheet->setTitle('Cambios en rol '.$mina_nom);
 ...
 $objSheet->getCell('E2')->setValue('Entrada');
 $objSheet->getCell('F2')->setValue('Hora Jornada');
 ...
 $i = 3;
  while( $row = sqlsrv_fetch_array( $eds_datos) ) { 
    ...
    $objSheet->getCell('E'.$i)->setValue($row['entrada']);
    $objSheet->getCell('F'.$i)->setValue($row['hr_ent_jor']);
 $i=$i+1; 
 }
 ...
 header('Content-Disposition: attachment;filename="Cambios en rol '.$mina_nom.'.xlsx"');
