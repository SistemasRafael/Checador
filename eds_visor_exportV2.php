...
$objSheet->setTitle('Checador '.$mina_nom);
 ...
 $objSheet->getCell('D2')->setValue('Checador');
 $objSheet->getCell('E2')->setValue('Fecha');
 $objSheet->getCell('F2')->setValue('Hora');
 ...
 $i = 3;
  while( $row = sqlsrv_fetch_array( $eds_datos) ) { 
    ...
    $objSheet->getCell('D'.$i)->setValue($row['terminal']);
    $objSheet->getCell('E'.$i)->setValue($row['fecha']);
    $objSheet->getCell('F'.$i)->setValue($row['hora']);
 $i=$i+1; 
 }
